...
{
	SYS_ASSERT(sizeof(pRotateStruct->oldAngle) == 2);
	fread(&pRotateStruct->oldAngle, 2, 1, fHandle);

	SYS_ASSERT(sizeof(pRotateStruct->newAngle) == 2);
	fread(&pRotateStruct->newAngle, 2, 1, fHandle);

	SYS_ASSERT(sizeof(pRotateStruct->param) == 2);
	fread(&pRotateStruct->param, 2, 1, fHandle);

	SYS_ASSERT(sizeof(pRotateStruct->timeOfRotate) == 2);
...
int loadSave(int saveNumber) 
{
	...
	SYS_ASSERT(sizeof(currentDisplayedRoom) == 2);
	fread(&currentDisplayedRoom, 2, 1, fHandle);

	SYS_ASSERT(sizeof(currentEtage) == 2);
	fread(&currentEtage, 2, 1, fHandle);

	SYS_ASSERT(sizeof(currentCamera) == 2);
	fread(&currentCamera, 2, 1, fHandle);

	SYS_ASSERT(sizeof(currentCameraTarget) == 2);
	fread(&currentCameraTarget, 2, 1, fHandle);

	SYS_ASSERT(sizeof(currentActorCameraTarget) == 2);
	fread(&currentActorCameraTarget, 2, 1, fHandle);

	SYS_ASSERT(sizeof(maxObjects) == 2);
	...
	{
		SYS_ASSERT(sizeof(objectTable[i].ownerIdx) == 2);
		fread(&objectTable[i].ownerIdx, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].body) == 2);
		fread(&objectTable[i].body, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].flags) == 2);
		fread(&objectTable[i].flags, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].zvType) == 2);
		fread(&objectTable[i].zvType, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].foundBody) == 2);
		fread(&objectTable[i].foundBody, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].foundName) == 2);
		fread(&objectTable[i].foundName, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].flags2) == 2);
		fread(&objectTable[i].flags2, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].foundLife) == 2);
		fread(&objectTable[i].foundLife, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].x) == 2);
		fread(&objectTable[i].x, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].y) == 2);
		fread(&objectTable[i].y, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].z) == 2);
		fread(&objectTable[i].z, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].alpha) == 2);
		fread(&objectTable[i].alpha, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].beta) == 2);
		fread(&objectTable[i].beta, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].gamma) == 2);
		fread(&objectTable[i].gamma, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].stage) == 2);
		fread(&objectTable[i].stage, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].room) == 2);
		fread(&objectTable[i].room, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].lifeMode) == 2);
		fread(&objectTable[i].lifeMode, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].life) == 2);
		fread(&objectTable[i].life, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].field_24) == 2);
		fread(&objectTable[i].field_24, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].anim) == 2);
		fread(&objectTable[i].anim, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].frame) == 2);
		fread(&objectTable[i].frame, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].animType) == 2);
		fread(&objectTable[i].animType, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].animInfo) == 2);
		fread(&objectTable[i].animInfo, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].trackMode) == 2);
		fread(&objectTable[i].trackMode, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].trackNumber) == 2);
		fread(&objectTable[i].trackNumber, 2, 1, fHandle);

		SYS_ASSERT(sizeof(objectTable[i].positionInTrack) == 2);
	...
	if(g_fitd->getGameType() == GType_AITD1) {
		//SYS_ASSERT(numCVars == 45);
	}

	for(i = 0; i < g_fitd->getNumCVars(); i++) {
		SYS_ASSERT(sizeof(CVars[i]) == 2);
		fread(&CVars[i], 2, 1, fHandle);
	}

	SYS_ASSERT(sizeof(inHand) == 2);
	fread(&inHand, 2, 1, fHandle);

	SYS_ASSERT(sizeof(numObjInInventory) == 2);
	fread(&numObjInInventory, 2, 1, fHandle);

	if(g_fitd->getGameType() == GType_AITD1) {
		SYS_ASSERT(INVENTORY_SIZE == 30);
	}

	for(i = 0; i < INVENTORY_SIZE; i++) {
		SYS_ASSERT(sizeof(inventory[i]) == 2);
		fread(&inventory[i], 2, 1, fHandle);
	}

	SYS_ASSERT(sizeof(statusScreenAllowed) == 2);
	fread(&statusScreenAllowed, 2, 1, fHandle);

	SYS_ASSERT(sizeof(giveUp) == 2);
	fread(&giveUp, 2, 1, fHandle);

	SYS_ASSERT(sizeof(lightVar1) == 2);
	fread(&lightVar1, 2, 1, fHandle);

	SYS_ASSERT(sizeof(shakingAmplitude) == 2);
	fread(&shakingAmplitude, 2, 1, fHandle);

	SYS_ASSERT(sizeof(shakeVar1) == 2);
	fread(&shakeVar1, 2, 1, fHandle);

	uint32 tempTimer;
	SYS_ASSERT(sizeof(g_fitd->getTimer()) == 4);
	fread(&tempTimer, 4, 1, fHandle);
	g_fitd->setTimer(tempTimer);

	SYS_ASSERT(sizeof(timerFreeze1) == 4);
	fread(&timerFreeze1, 4, 1, fHandle);

	SYS_ASSERT(sizeof(currentMusic) == 2);
